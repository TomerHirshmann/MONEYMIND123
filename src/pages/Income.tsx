...
import { Plus, ArrowUpRight, TrendingUp, DollarSign, AlertCircle } from 'lucide-react';
...
const Income: React.FC = () => {
  ...
  const loadTransactions = async () => {
    ...
    try {
      setLoading(true);
      const data = await getTransactions(user.uid, 'income');
  ...
  const handleNewIncome = async (transaction: any) => {
    if (!user) return;
    try {
      if (transaction.id) {
  ...
  const currentMonthIncome = transactions.reduce((sum, t) => {
  ...
  const previousMonthIncome = transactions.reduce((sum, t) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">הכנסות</h1>
        <button
          onClick={() => {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
          ...
          <div className="mt-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-900 dark:text-white">{formatAmount(currentMonthIncome)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600 dark:text-gray-400">חודש קודם</span>
              <span className="font-medium text-gray-900 dark:text-white">{formatAmount(previousMonthIncome)}</span>
            </div>
            <div className="h-2 bg-gray-100 dark:bg-gray-700 rounded-full mt-2">
              <div
                className="h-full bg-success-500 rounded-full"
                style={{ width: `${(currentMonthIncome / Math.max(currentMonthIncome, previousMonthIncome)) * 100}%` }}
              />
            </div>
          </div>
        </div>
